...
- **Share, rename and locate**: send the open file to any app, rename one in an added folder or picked from the phone's storage or Downloads, or jump to its folder in the file manager
...
Rename uses the grants the picker hands over, which cover changing a file as well
as reading it. Gander keeps both for a folder you add and for a picked file it can
rename, and gives them back when you remove the folder or the file leaves Recents.
