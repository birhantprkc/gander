...
#
# The first part is cut by counting frames, not by -t alone: copying streams, -t lets through
# the two packets that were reordered across the mark, and those are the first two frames of
# the end card. Two frames of end card at the tail of the film, and a Story of 60.07 seconds,
# which Instagram would cut again.
set -eu
rate=$(ffprobe -v error -select_streams v:0 -show_entries stream=r_frame_rate -of csv=p=0 "$1")
frames=$(( 60 * ${rate%/*} / ${rate#*/} ))
ffmpeg -y -loglevel error -i "$1" -t 60 -frames:v "$frames" -c copy -movflags +faststart "$2-1-film.mp4"
